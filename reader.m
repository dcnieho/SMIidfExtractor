...
while true
    if any(type=='SM')
        ...
        timeStamp = typecast(temp([5:8 1:4]),'uint64');
    end
    switch char(type)
        case 'M'
        ...
        case 'S'
            ...
            samples.setNum.append(double(dataStream.readType(1,'uint32')));
            ...
            if version==9
                ...
                dataStream.seek(10);
                % pupil confidence
                samples.pupConf.append(double(dataStream.read(1)));
            elseif version==8
            ...
            for e=1:(dataInfo.hasLeft+dataInfo.hasRight)
                ...
                if dataInfo.hasPupRaw
                    samples.(eye).rawPup.append(dataStream.readType(2,'double'));
                end
                % Pupil diameter
                if dataInfo.hasPupDiam
                    samples.(eye).pupDiam.append(dataStream.readType(2,'double'));
                end
                % CRs
                for c=1:dataInfo.nCR
                    samples.(eye).(CRnames{c}).append(dataStream.readType(2,'double'));
                end
            end
            % POR data
            for e=1:(dataInfo.hasLeft+dataInfo.hasRight)
                ...
                % point of regard
                if dataInfo.hasPOR
                    samples.(eye).gaze.append(dataStream.readType(2,'double'));
                end
                % if remotes, seems to have a plane per eye. TODO: Part of
                % hasEyeball?
                if dataInfo.hasEyeball
                    % plane
                    samples.(eye).plane.append(double(dataStream.readType(1,'int32')));
                    % seems one more double of unknown use, ignore
                    dataStream.seek(8);
                end
            end
            % head 6dof data, position
            if dataInfo.hasHeadPos
                samples.headPos.append(dataStream.readType(3,'double'));
            end
            % head 6dof data, orientation
            if dataInfo.hasHeadOri
                samples.headOri.append(dataStream.readType(3,'double'));
            end
            ...
            if dataInfo.hasEyeball
                for e=1:(dataInfo.hasLeft+dataInfo.hasRight)
                    ...
                    % eye position
                    samples.(eye).eyePos.append(dataStream.readType(3,'double'));
                    % gaze vector
                    samples.(eye).gazeVec.append(dataStream.readType(3,'double'));
...
recording.samples       = getSamples(samples);