...
while true
    ...
    switch type
        case 'M'
            nChar = fread(fid, 1, 'uchar');
            msgs(end+1,:) = {timeStamp,fread(fid, nChar, '*char').'};
        ...
        case 'S'
            ...
            for e=1:(dataInfo.hasLeft+dataInfo.hasRight)
                ...
                if dataInfo.hasPupRaw
                    samp.(eye).rawPupX = fread(fid, 1, 'double');
                    samp.(eye).rawPupY = fread(fid, 1, 'double');
                end
                % Pupil diameter
                if dataInfo.hasPupDiam
                    samp.(eye).pupXDiam = fread(fid, 1, 'double');
                    samp.(eye).pupYDiam = fread(fid, 1, 'double');
                end
                % CRs
                for c=1:dataInfo.nCR
                    samp.(eye).rawCRX(c) = fread(fid, 1, 'double');
                    samp.(eye).rawCRY(c) = fread(fid, 1, 'double');
                end
            end
            % POR data
            for e=1:(dataInfo.hasLeft+dataInfo.hasRight)
                ...
                % point of regard
                if dataInfo.hasPOR
                    samp.(eye).GX = fread(fid, 1, 'double');
                    samp.(eye).GY = fread(fid, 1, 'double');
                end
            ...
            if dataInfo.hasHeadPos
                samp.HX = fread(fid, 1, 'double');
                samp.HY = fread(fid, 1, 'double');
                samp.HZ = fread(fid, 1, 'double');
            end
            if dataInfo.hasHeadOri
                samp.Ha = fread(fid, 1, 'double');
                samp.Hb = fread(fid, 1, 'double');
                samp.Hc = fread(fid, 1, 'double');
            end
            ...
            if dataInfo.hasEyeball
                for e=1:(dataInfo.hasLeft+dataInfo.hasRight)
                    ...
                    samp.(eye).eyePosX  = fread(fid, 1, 'double');
                    samp.(eye).eyePosY  = fread(fid, 1, 'double');
                    samp.(eye).eyePosZ  = fread(fid, 1, 'double');
                    samp.(eye).gazeVecX = fread(fid, 1, 'double');
                    samp.(eye).gazeVecY = fread(fid, 1, 'double');
                    samp.(eye).gazeVecZ = fread(fid, 1, 'double');
